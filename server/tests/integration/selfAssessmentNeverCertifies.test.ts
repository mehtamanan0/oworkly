...
describe("self-assessment never certifies", () => {
  ...
  it("a perfect-score self-assessment quiz never creates a qualification_result, changes case status, or issues a certificate", async () => {
    ...
    const selfComponentAttemptId = attemptDetail.body.componentAttempts[0].assessment_attempt_section_id;
    ...
    const itemsRes = await agent.get(`/api/v1/v2/assessment-attempt-sections/${selfComponentAttemptId}/items`).set("Authorization", `Bearer ${workerToken}`);
    const responses = await Promise.all(
      itemsRes.body.map(async (it: any) => ({
        ...
        responseJson: { chosen: await getCorrectOptionKey(it.question_id) },
      }))
    );
    const scoreRes = await agent
      .post(`/api/v1/v2/assessment-attempt-sections/${selfComponentAttemptId}/score`)
      .set("Authorization", `Bearer ${workerToken}`)
      .send({ responses });
  ...
  it("a worker cannot score a Practical/Theory/Behavioural component even if asked to (no SELF capacity over them)", async () => {
    ...
    const workerToken = await verifyAsWorker(fixtures);
    const practicalDef = await pool.query(`SELECT assessment_id FROM assessment WHERE name = 'BRA Practical Evaluation'`);
    ...
    const items = await pool.query(`SELECT question_id, max_score FROM question WHERE assessment_id = $1 AND is_active AND review_status = 'approved'`, [practicalDef.rows[0].assessment_id]);
    ...
    const pkg = await pool.query(`SELECT assessment_template_assessment_id FROM assessment_template_assessment WHERE assessment_id = $1 LIMIT 1`, [practicalDef.rows[0].assessment_id]);
    const qcaseRes = await agent
      .post("/api/v1/v2/qualification-cases")
      .set("Authorization", `Bearer ${workerToken}`)
      .send({ workerId: fixtures.rajeshWorkerId, processId: fixtures.braProcessId, targetProcessLevelId: fixtures.levelIdByCode.E3 });
    const attemptRow = await pool.query(
      `INSERT INTO assessment_attempt (qualification_case_id, assessment_template_id, worker_id, attempt_no, status, started_at)
       VALUES ($1,$2,$3,99,'in_progress',now()) RETURNING assessment_attempt_id`,
      [qcaseRes.body.qualification_case_id, qcaseRes.body.assessment_template_id, fixtures.rajeshWorkerId]
    );
    const componentAttempt = await pool.query(
      `INSERT INTO assessment_attempt_section (assessment_attempt_id, assessment_template_assessment_id, status) VALUES ($1,$2,'pending') RETURNING assessment_attempt_section_id`,
      [attemptRow.rows[0].assessment_attempt_id, pkg.rows[0].assessment_template_assessment_id]
    );

    const res = await agent
      .post(`/api/v1/v2/assessment-attempt-sections/${componentAttempt.rows[0].assessment_attempt_section_id}/score`)
      .set("Authorization", `Bearer ${workerToken}`)
      .send({ responses: items.rows.map((it) => ({ questionId: it.question_id, score: it.max_score })) });
